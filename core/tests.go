...
func RunTestScenario(ctx context.Context, scenario *TestScenario, executor *CrewExecutor) *TestResult {
	startTime := time.Now()
...
func recordAgentFlow(response *CrewResponse) []string {
	var flow []string
	if response != nil {
...
func validateAssertions(scenario *TestScenario, response *CrewResponse, flow []string) []string {
	var errors []string
