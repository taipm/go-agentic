...
func RouteBySignal(signalName string, routing *common.RoutingConfig) (string, error) {
	if routing == nil {
		return "", common.NewRoutingError("routing configuration is nil")
	}

	if signalName == "" {
		return "", common.NewValidationError("signal_name", "signal name is empty")
	}

	if routing.Signals == nil {
		return "", common.NewRoutingError("no signals configured in routing")
	}
	...
	return "", common.NewRoutingErrorf("signal '%s' not found in routing configuration", signalName)
}
...
func RouteByBehavior(behavior string, routing *common.RoutingConfig) (string, error) {
	...
	if behavior == "" {
		return "", common.NewValidationError("behavior", "behavior name is empty")
	}

	if routing.AgentBehaviors == nil || len(routing.AgentBehaviors) == 0 {
		return "", common.NewRoutingError("no agent behaviors configured in routing")
	}

	// Lookup behavior in routing configuration
	behaviorConfig, exists := routing.AgentBehaviors[behavior]
	if !exists {
		return "", common.NewBehaviorNotFoundError(behavior)
	}
