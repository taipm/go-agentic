## Description

Please include a summary of the changes and related context.

## Type of Change

Please select the relevant options:

- [ ] Bug fix (non-breaking change which fixes an issue)
- [ ] New feature (non-breaking change which adds functionality)
- [ ] Breaking change (fix or feature that would cause existing functionality to not work as expected)
- [ ] Documentation update
- [ ] Dependency update
- [ ] Refactoring
- [ ] Performance improvement
- [ ] Security fix

## Related Issues

Fixes # (issue number)

## How Has This Been Tested?

Please describe the tests you ran and how to reproduce them.

## Checklist

Before submitting this PR, ensure:

- [ ] My code follows the style guidelines of this project
- [ ] I have performed a self-review of my own code
- [ ] I have commented my code, particularly in hard-to-understand areas
- [ ] I have made corresponding changes to the documentation
- [ ] My changes generate no new warnings
- [ ] I have added tests that prove my fix is effective or that my feature works
- [ ] New and existing unit tests passed locally with my changes
- [ ] I have verified there are no hardcoded credentials in my changes
- [ ] I have checked for any security vulnerabilities

## Security Checklist

- [ ] No hardcoded API keys, passwords, or tokens
- [ ] Environment variables are used for sensitive data
- [ ] No new dependencies without security review
- [ ] Dependencies are properly locked in go.mod
- [ ] No changes to security-sensitive files without review

## Performance Impact

- [ ] No breaking changes to public API
- [ ] Performance optimizations if applicable
- [ ] Memory usage is acceptable
- [ ] No new goroutine leaks

## Additional Notes

Add any other context about the PR here.

---

**Note**: All PRs must pass the following checks before merge:
- ✅ Tests pass (with race detection)
- ✅ Security scan passes (no hardcoded secrets)
- ✅ Code quality checks pass (golangci-lint)
- ✅ Dependencies verified (no vulnerabilities)
